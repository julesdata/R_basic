...
rm(x)  #x변수 삭제 ## 새로운 변수 사용시 덮어씌워져서 사용할 일이 별로 없지만, 너무 많은 데이터가 있어 무거워졌을때 사용함. 
...
v3 <- c(T, F, T)    #R에서는 T,F를 워낙 많이 사용해서 자동적으로 함수로 인식. 변수 지정하면 안됨
height <- c(160, 140, 155)     #height 벡터 생성


## 이건 왜 ????
my_variable <- c(10)
my_varıable  #오타떄문에.. 대/소문자 차이도 구별함. 
...
# 1부터 100까지 짝수를 2번씩 출력

rep(seq(2,100,2),each=2)

# 1부터 5까지 숫자만큼 반복 출력
rep(1:5,times=1:5) # 'times =' 생략가능 


# rep, seq를 한번에 사용하는 문제
...
# 요인(범주형 자료)료
#factor(obj, levels=변수값, labels=요인이름) , level지정안할시 값 내에서 자동생성 
#2PPT - 16
...
rownames(mat) <- c("a1","a2","a3") #행이름 지정
mat
colnames(mat) <- c("z1","z2","z3")  #열 이름 지정
mat
...
char1 <- rep(LETTERS[1:3],c(2,2,1))          #벡터  #Letter: 영어 대문자 함수 
char1
num1 <- rep(1:3,c(2,2,1))                     #벡터
num1
test1 <- data.frame(char1, num1)              #test1 데이터 프레임 생성, 열기준으로 정렬
test1
...
li[[1]]                                        #[[1]]:List의 데이터 접근은대괄호 두개:첫 번째 성분
